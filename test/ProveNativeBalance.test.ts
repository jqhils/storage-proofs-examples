...
describe("ProveNativeBalance", () => {
    describe("deployment", () => {
        it("should deploy the contract", async () => {
            const contractFactory = await ethers.getContractFactory("ProveNativeBalance");
    ...
    describe("proving and verifying native balance on-chain", () => {
        ...
        // Generate a proof
        const { noir, backend } = await hre.noir.getCircuit("native_balance", UltraPlonkBackend);
        ...
        let witnessData = fs.readFileSync(`./test/inputs/TestNativeBalanceProver.toml`);
        let input = toml.parse(witnessData);
        fs.writeFileSync('./test/inputs/native-balance-input-data.json', JSON.stringify(input, null, 2));
